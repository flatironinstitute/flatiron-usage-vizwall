...
async function fetchData(queryObj: QueryObject, isRange: boolean) {
  ...
  if (isRange) {
    let end = moment().subtract(10, 'minutes').toISOString();
    let start = moment().subtract(queryObj.amount, queryObj.unit).toISOString();
...
function combineBubbleData(waittimes: [any], queuelengths: [any]) {
  ...
  let shorter = waittimes.length < queuelengths.length ? waittimes.length : queuelengths.length;
...
function getBubbleplotData() {
  ...
  for (let i = 0; i < waitTimes.length; i++) {
    if (waitTimes[i].metric.account === queueLengths[i].metric.account) {
      let datamap = combineBubbleData(waitTimes[i].values, queueLengths[i].values);
